...
import type { DefaultCompanyDirectoryEntry } from "../src/modules/job-market/application/default-company-directory";
import companyDirectoryAliases from "../src/modules/job-market/application/company-directory-aliases.json";
import recentWechatArticles from "../src/modules/job-market/application/recent-wechat-articles.json";
import researchedOfficialSites from "../src/modules/job-market/application/researched-official-sites.json";

// 公司目录的唯一数据源：目录数据与组装逻辑维护在本脚本，产出
// default-company-directory.json 供运行时导入。目录按当前 catalog JSON
// 与公众号数据实时组装，任何输入变化后都需运行 pnpm company-directory:generate；
// CI 用 company-directory:check 校验 JSON 新鲜度。
const OUTPUT_PATH =
  "src/modules/job-market/application/default-company-directory.json";
...
const DEFAULT_COMPANY_DIRECTORY = [
  ...curatedDirectory,
  ...new Map(
...
const payload = { directory: DEFAULT_COMPANY_DIRECTORY };
...
async function main() {
  if (process.argv.includes("--check")) {
    ...
    if (current !== serialized) {
      console.error(
        `default-company-directory.json 与脚本数据不一致，请运行 pnpm company-directory:generate 并提交。`,
      );
      process.exit(1);
    }
    console.log("default-company-directory.json up to date");
    return;
  }
  await writeFile(OUTPUT_PATH, serialized, "utf8");
  console.log(`wrote ${OUTPUT_PATH} (${payload.directory.length} entries)`);
}
