...
生产默认目录需要通过唯一标识、HTTPS URL 和主机白名单一致性测试；持久化测试需要验证重复初始化不会产生重复数据，并保留管理员设置的暂停/撤销状态。不要将目录验证改为 CI 实时联网测试：ATS 可用性属于外部不确定因素。目录变更前人工验证候选端点，适配器行为继续通过 `tests/fixtures/job-market/` 下的固定数据覆盖。
