...
数据库性能基准使用可重复种子，并覆盖大数据场景。`tests/performance/repository-performance.ts` 直接执行生产仓储形状，使用 10,000 条私人记录和 100 家企业/100,000 个岗位验证应用列表、面经列表、完整分析报告与岗位市场列表；这些读取的 p95 门限为 500ms。当前管理后台读取 p95 门限为 2 秒、写入 p95 门限为 1 秒；面经主要列表、筛选、搜索和聚合更新 p95 门限为 1 秒。

分析报表回归必须覆盖空集合、组合筛选、上一周期对比和 owner 隔离。岗位市场读模型变更必须同时验证空库迁移重放、排除/包含关闭岗位两种投影、成功同步事务内刷新、来源停用后的刷新，以及从规范化表全量重建；不要把私人收藏或投递状态写入公共投影。
