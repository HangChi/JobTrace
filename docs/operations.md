...
管理员可以打开 `/admin/job-market` 并点击“一键初始化并首次同步”。当前目录包含 1 家国内企业和 7 家外企的中国大陆招聘来源，已于 2026-08-31 验证。SmartRecruiters 来源使用 `country=cn`，小米使用官网公开调用的国内岗位接口；每家公司每次最多保留最新 100 个返回岗位，超出时运行状态为 `partial`。该操作会：
