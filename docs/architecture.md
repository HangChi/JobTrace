...
投递和面经列表先从 owner 过滤后的集合中计算精确总数并选出当前页，再只对这一页聚合阶段、问题和行动项。游标分页继续返回完整筛选集合的总数；超出范围的页也保留总数。周期报告把一个时间范围的投递、阶段和面经归并为一份物化事实集，一次数据库任务返回该范围的全部指标、趋势和维度；启用对比周期时整份报告最多执行两个数据库任务。
...
默认企业来源目录位于 `src/modules/job-market/application/default-source-catalog.ts`。它属于受审查的出站来源配置，而不是数据库 seed；当前包含 203 家可自动同步企业、207 个来源，并保持中国企业占多数。管理员初始化服务先验证每个 HTTPS 来源，再通过 `PostgresSourceCatalogRepository` 幂等持久化企业和来源，最后复用正常的来源认领与同步管线。因此，默认目录、手工登记和定时任务会产生相同的标准化岗位、生命周期事件、安全诊断与日志。
...
每个适配器只接受管理员登记的来源，并输出统一的完整或部分批次。来源可持久化 ISO 两位国家代码范围；SmartRecruiters 在服务端请求中应用该范围，Greenhouse 与 Lever 在标准化前按中国大陆地点白名单过滤，Moka 适配器分页读取公开招聘官网接口并按社招/校招构造官方投递页，小米专用适配器访问官网岗位接口后也按中国大陆城市白名单执行失败关闭式过滤；美团使用公开职位列表 API，中广核使用不执行脚本的 Dayee 移动职位卡片解析。需要前端混淆签名的飞书门户和明确限制服务器 IP 的个别 Moka 租户不再作为默认自动来源，只保留官方目录入口。默认中国市场目录只通过显式稳定企业身份把同一主体的多来源统一到公司卡片，不会按规范化显示名称合并同名法人；首页默认不聚合已经关闭的历史岗位，但“已失效”筛选和当前用户收藏视图可保留关闭历史。新增适配器必须加入显式注册表，提供本地 fixture 契约测试，并遵守 HTTPS、精确主机白名单、DNS 公网地址、逐跳重定向、超时、响应大小和内容类型限制。Schema.org 适配器只解析 JSON-LD 和纯文本，不执行来源脚本。

去重依次使用同来源外部 ID、同公司规范官方 URL 和保守指纹；模糊或有歧义的数据保持分离并保留来源记录。日常同步以增量变化落库：新岗位新增，未变化岗位保留原业务字段、来源发布时间和 `updated_at`，只有来源内容或状态确实变化时才更新；完整成功快照第一次缺失变为 `stale`，至少六小时后的第二次缺失才 `closed`，失败或部分批次不推进缺失状态，重新出现记录 `reopened`。首页始终按公司返回一条聚合记录，合并其岗位与地点，并以该公司当前岗位中最新的来源发布时间作为用户可见更新日期，不使用同一批同步确认时间覆盖既有日期。
...
公司浏览列表从 `job_market_company_read_models` 读取两种可见模式（排除或包含已关闭岗位）。该表只保存可由公共公司、活动、岗位、地点和已启用来源重建的摘要，不保存收藏等 owner 数据；查询时再合并当前用户收藏。成功同步在提交岗位、同步运行和租约状态的同一事务中刷新对应公司，来源状态与默认目录变更也刷新投影。规范化业务表始终是权威来源，可调用 `rebuild_job_market_company_read_models()` 全量重建投影。
