...
首次部署并创建管理员后，访问 `/admin/job-market`，点击“一键初始化并首次同步”。初始化作为后台任务执行并显示实时进度，可以离开页面；完成后会创建默认公司和活动来源，之后服务器定时器会持续认领到期来源。当前默认目录规模（475 个来源）超过单次同步窗口的上限（30 批 × 每批 10 个），全量到期时会顺延到后续六小时窗口逐步追平。
