...
export function CampaignCard({ campaign }: { campaign: CampaignSummary }) {
  ...
  return (
    <article className={`campaign-card status-${campaign.status}`}>
      ...
      {isDirectory ? (
        <div className="campaign-directory-note">
          <strong>公众号招聘目录</strong>
          <span>岗位与地点以公众号最新发布为准，系统不采集封闭内容。</span>
        </div>
      ) : (
      ...
      <footer className="campaign-footer">
        <div className="campaign-meta">
          ...
          >
            {isDirectory ? "招聘入口" : "官方来源"}：{campaign.source.name}
            <ExternalLinkIcon />
          </a>
          <span>
            {isDirectory ? "目录入口，不代表岗位已自动同步" : `更新于 ${date}`}
          </span>
        </div>
