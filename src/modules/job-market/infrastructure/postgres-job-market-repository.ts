  ...
  async completeBatch(
    ...
    const [result] = await this.sql<BatchResult[]>`
      ...
      from public.complete_job_market_sync_success(
        ...
        ${now},
        ${claim.source.syncIntervalMinutes},
        ${status},
        ${batch.sourceMetadata.etag ?? null},
        ${batch.sourceMetadata.lastModified ?? null}
      )`;
    if (!result) throw new Error("sync_completion_missing_result");
    return result;
  }
}
