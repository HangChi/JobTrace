...
**Decision**: 在来源认领事务中创建运行记录，并将运行 ID 作为来源租约的 fencing token。成功路径由数据库函数在一个事务中校验 token、应用岗位批次、完成运行、更新来源健康和释放租约；失败路径同样以 token 为条件原子完成运行与退避。租约过期重领以及暂停或撤销来源都会先终止旧运行。

**Rationale**: 仅依赖到期时间和共享 worker 标识无法阻止超时 worker 在新认领之后继续写入，也会在进程退出时留下“岗位已更新但运行仍在执行”等拆分状态。每次认领唯一的运行 ID 同时提供可审计的所有权与数据库级陈旧写入拒绝。

**Alternatives considered**: 仅增加事务但不加 fencing token 仍允许旧 worker 在租约重领后提交；使用 worker ID 作为 token 无法区分同一实例的连续运行；在应用层按顺序更新多个表无法覆盖进程退出和并发重领。
