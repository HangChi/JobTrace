...
1. 为公司 A 提供一个统一安全官方入口；为公司 B 提供统一入口和多个仅供追溯的岗位地址；为公司 C 提供无链接、HTTP 链接或已知失效链接。
2. 在列表中操作“立即投递”。

Expected:

- A 不进入详情，直接用带 `noopener,noreferrer` 的新页面打开官方 HTTPS 地址。
- B 直接打开唯一公司官方入口，不显示岗位地址选择器，也不产生重复公司行；岗位地址仍可在详情追溯。
...
Expected: 企业以单张“公众号招聘目录”卡片显示；岗位和地点不被虚构，页面明确说明其不是自动同步结果，“查看原文”会打开已审核的公众号招聘原文，并且该企业不会出现在自动来源健康列表中。

## Scenario 8.2: Source discovery and review

1. 在企业目录中准备一个公开招聘官网，页面链接到固定 fixture 的受支持 ATS；再准备一个可访问但无法识别的页面和一个超时页面。
2. 管理员在 `/admin/job-market` 点击“扫描下一批”。
3. 核对识别类型、置信度、精确主机、健康诊断，然后批准健康候选并忽略未识别记录。

Expected: 三个入口互不影响；只有识别成功的入口进入“待审核”，扫描本身不增加活动来源。管理员批准后创建一个立即到期的活动来源，未识别或失败入口不能批准；普通用户和非管理员不能读取或操作候选。
...
部署环境每 6 小时调用内部同步接口并分批覆盖全部活动来源；运行时密钥由秘密管理系统注入，不能放在 cron 配置日志或仓库。告警至少覆盖连续失败、12 小时未成功、租约长期未释放和单次数据量突变。健康检查不应依赖任何外部招聘源在线。
