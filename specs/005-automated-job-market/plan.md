...
在现有 Next.js 单体应用中新增独立的 `job-market` 业务模块，从已登记且合规的公开 ATS API、授权接口和受限的 Schema.org `JobPosting` 页面自动同步岗位。公共岗位与私人投递严格分表管理；首页改为“招聘广场”，以企业为一条记录，合并岗位和地点，并提供每家公司唯一的官方招聘入口。同步由外部调度器周期调用受密钥保护的内部 Route Handler，数据库租约保证多实例安全；来源失败只影响该来源，现有数据保持可读。
...
- `job-market/ui`: 招聘广场、聚合行/卡片、筛选器、公司官方入口及管理员同步视图。
...
- 每家公司只展示一个经过校验的官方招聘入口并直接新窗口打开；岗位级链接只用于详情追溯，不拆行、不提供岗位地址选择器。
