...
| Gate | 设计响应 | 结果 |
|------|----------|------|
| 可维护代码与最小复杂度 | 复用现有 Supabase 平台的 Auth，新增单一 identity-access 模块；无自研密码存储、微服务或消息总线 | PASS |
...
- 认证表单使用 Server Actions、Zod 与 `useActionState`；错误提示不区分“邮箱不存在”和“密码错误”，支持密码管理器、键盘和可感知的提交状态。
...
- **Authorization integration**: profiles 创建触发器、owner 回填、RLS、普通用户跨 owner 拒绝、管理员操作、自我锁死保护、禁用后会话拒绝。
