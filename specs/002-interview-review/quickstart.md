...
使用事务内固定种子生成每用户最多 10,000 篇面经，至少预热后测量三轮；无论成功或失败均回滚，不向配置数据库提交测试数据：
...
> 2026-08-21 增量复核：Vitest 当前为 34 个文件、86 项通过；契约 10/10、集成 16/16，面经 10,000 条 raw 基准 p95 为列表 26.72ms、筛选 26.22ms、搜索 35.03ms、更新 29.27ms。性能种子已改为事务回滚，不再写入配置数据库。
