-- A new application without an explicit stage starts at resume screening.
-- Explicit non-empty stage history remains authoritative.
...
declare
  ...
  stage_item jsonb;
  stage_date date;
  resolved_stages jsonb;
begin
  if payload ? 'stages' and jsonb_typeof(payload->'stages') <> 'array' then
    raise exception using errcode='22023',message='invalid_stages';
  end if;

  resolved_stages := coalesce(payload->'stages','[]'::jsonb);
  if jsonb_array_length(resolved_stages)=0 then
    resolved_stages := jsonb_build_array(jsonb_build_object(
      'stage','screening','occurredOn',payload->>'appliedDate'
    ));
  end if;
  ...
  for stage_item in select value from jsonb_array_elements(resolved_stages) loop
